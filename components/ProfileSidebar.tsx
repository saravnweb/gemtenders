...
export default function ProfileSidebar({ user, onClose }: { user: any, onClose?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-slate-200 w-full md:w-64 shadow-sm">
      {/* Sidebar Header - Mobile Close */}
      <div className="p-4 border-b border-slate-100 flex items-center justify-between md:hidden">
        <span className="font-bold text-fresh-sky-950">Menu</span>
      ...
      <div className="p-6 bg-slate-50/50 border-b border-slate-200">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wide">Member Account</span>
          </div>
        </div>
      </div>
            ...
            <Link
              ...
              className={`flex items-center justify-between px-3 py-2.5 rounded-xl transition-all group ${
                isActive 
                  ...
                  : "text-slate-600 hover:bg-slate-50 hover:text-fresh-sky-950"
              }`}
            >
              <div className="flex items-center space-x-3">
                <item.icon className={`w-4 h-4 ${isActive ? "text-blue-400" : "text-slate-400 group-hover:text-blue-500"}`} />
      ...
      <div className="p-4 border-t border-slate-200">
        <button
          ...
          className="flex items-center space-x-3 w-full px-4 py-3 text-red-500 font-bold text-xs uppercase tracking-widest hover:bg-red-50 rounded-xl transition-all"
        >
