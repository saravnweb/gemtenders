      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">Manage your search monitors and saved opportunities.</p>
          </div>
          <div className="flex items-center space-x-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm">
             ...
             <span className="text-[10px] font-bold text-slate-600 uppercase">{membershipPlan}</span>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
               <div className="flex flex-col sm:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                     ...
                     <div className="flex flex-wrap justify-center sm:justify-start gap-x-4 gap-y-1 text-slate-400 font-medium text-xs">
                  ...
                  <button className="w-full sm:w-auto px-5 py-2 bg-slate-50 text-slate-700 font-bold text-xs uppercase tracking-widest rounded-lg border border-slate-200 hover:bg-slate-100 transition-all">
                     Edit Profile
                  </button>
               </div>
            </div>
          ...
          <div className="lg:col-span-1 space-y-6">
             <div className="bg-slate-900 rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <Link href="/dashboard/subscriptions" className="flex items-center justify-between bg-white text-slate-900 px-4 py-3 rounded-lg transition-all hover:bg-slate-100 group">
...
function MetricCard({ icon, label, value, href, color }: { icon: any, label: string, value: number, href: string, color: 'blue' | 'orange' }) {
    return (
        <Link href={href} className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex items-center justify-between hover:border-slate-300 hover:shadow-md transition-all group">
            <div className="flex items-center gap-4">
               <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${color === 'blue' ? 'bg-blue-50' : 'bg-orange-50'}`}>
                  {icon}
               </div>
               <div>
            ...
            <div className="p-2 bg-slate-50 rounded-lg group-hover:bg-slate-900 group-hover:text-white transition-all border border-slate-100">
