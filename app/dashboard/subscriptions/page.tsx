...
export default function SubscriptionsPage() {
  ...
  if (!profile) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 space-y-4">
       <div className="w-10 h-10 border-4 border-slate-200 border-t-atomic-tangerine-500 rounded-full animate-spin"></div>
       <p className="text-sm font-bold text-slate-500 uppercase tracking-widest animate-pulse">Loading secure checkout...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 font-medium">Upgrade your plan to unlock more features.</p>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex items-center justify-between">
         ...
         <div className="text-right flex items-center space-x-3">
             ...
             <button 
                ...
                className="w-12 h-6 bg-slate-200 rounded-full flex items-center p-1 cursor-pointer transition-colors relative"
             >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Starter Plan */}
        <div className={`bg-white rounded-2xl p-6 border-2 transition-all ${profile.membership_plan === 'starter' ? 'border-atomic-tangerine-500 shadow-md ring-4 ring-atomic-tangerine-100' : 'border-slate-200 hover:border-slate-300'}`}>
           ...
           <p className="text-sm text-slate-500 h-10">Essential alerts and insights for freelancers.</p>
           <div className="my-6">
             <span className="text-4xl font-black text-slate-900">{isAnnual ? '₹79' : '₹99'}</span>
           ...
           <div className="flex flex-col mb-6">
             ...
             {isAnnual && profile.membership_plan !== 'starter' && profile.membership_plan !== 'pro' && (
               <span className="text-center text-[10px] text-slate-400 font-medium mt-2">Billed ₹948 once per year</span>
             )}
           </div>
        ...
        {/* Pro Plan */}
        <div className={`bg-white rounded-2xl p-6 border-2 transition-all ${profile.membership_plan === 'pro' ? 'border-amber-500 shadow-md ring-4 ring-amber-100' : 'border-slate-200 hover:border-slate-300'}`}>
           ...
           <p className="text-sm text-slate-500 h-10">Advanced features for power users and teams.</p>
           <div className="my-6">
             <span className="text-4xl font-black text-slate-900">{isAnnual ? '₹239' : '₹299'}</span>
             <span className="text-sm text-slate-500 font-bold">/mo</span>
           </div>
